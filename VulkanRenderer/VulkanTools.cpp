...
#undef STR
    ...
    std::string readTextFile(const char *fileName)
    {
        ...
        if (!fileStream.is_open()) {
            ILogger::Log(eLL_Error, "Vulkan Text File %s not found\n", fileName);
            return "";
        }
        std::string line = "";
...
VkBool32 vkTools::getMemoryType(
...
VkImageAspectFlags vkTools::getAspectFlags(
    VkFormat format)
{
    ...
    else if ((format >= VK_FORMAT_D16_UNORM_S8_UINT) && (format <= VK_FORMAT_D32_SFLOAT_S8_UINT)) {
