...
float r(int a, float b = 0, double base_range = 1000.0)
{
    return static_cast<float>(std::rand() % (int)(a * base_range) + b * base_range) / base_range;
}
...
struct Body
{
    ...
    {
        position += direction * static_cast<float>(dt);
        rotation += rotationd * dt;
    }
};

struct ParticleBodyGroup : public Body
{
    ParticleBodyGroup(size_t _nCount, const sf::Vector2f &position, const sf::Vector2f &direction, float rotationd = 0.0)
        : Body (position, direction, rotationd)
    {
        m_kBodys.resize(_nCount);
    }

    std::vector <Body> m_kBodys;

    virtual void update(float dt)
    {
        sf::Vector2f kPrepos = position;
        float fPreRotate = rotation;
        Body::update(dt);

        sf::Vector2f kVector = position - kPrepos;
        float fDeltaRot = rotation - fPreRotate;

        for (auto& kBody : m_kBodys) {
            kBody.position += kVector;
            kBody.position += kBody.direction * static_cast<float>(dt);
            kBody.rotation += fDeltaRot;
            kBody.rotation += rotationd * dt;
        }
    }
};
...
public:
    ...
    void update(ex::EntityManager &es, ex::EventManager &events, ex::entityx::TimeDelta dt) override
    {
        int c = 0;
        ...
        for (int i = 0; i < count - c; i++) {
            ...
            // "Physical" attributes.
            float fRandX = r(size.x, 0.0f, 1.0);
            float fRandY = r(size.y, 0.0f, 1.0);
            entity.assign<Body>(
                sf::Vector2f(fRandX, fRandY),
                sf::Vector2f(r(100, -50), r(100, -50)));

            // Shape to apply to entity.
            sf::Shape* shape = (new sf::CircleShape(collideable->radius));
            shape->setFillColor(sf::Color(r(128, 127, 1), r(128, 127, 1), r(128, 127, 1)));
...
struct BodySystem : public ex::System<BodySystem>
{
    void update(ex::EntityManager &es, ex::EventManager &events, ex::entityx::TimeDelta dt) override
    {
        ...
        es.each<ParticleBodyGroup>([dt] (ex::Entity entity, ParticleBodyGroup &body)
        {
            body.update(dt);
        });
    };
...
private:
    ...
    void collect(ex::EntityManager &entities)
    {
        entities.each<Body, Collideable>([this] (ex::Entity entity, Body &body, Collideable &collideable)
        {
            ...
            if (grid.size() > slots[0]) grid [slots [0]].push_back(candidate);
            if (slots [0] != slots [1]) if (grid.size() > slots [1]) grid [slots [1]].push_back(candidate);
            if (slots [1] != slots [2]) if (grid.size() > slots [2]) grid [slots [2]].push_back(candidate);
            if (slots [2] != slots [3]) if (grid.size() > slots [3]) grid [slots [3]].push_back(candidate);
        });
    }
...
public:
    ...
    void update(ex::EntityManager &es, ex::EventManager &events, ex::entityx::TimeDelta dt) override
    {
        ...
        es.each<Particle, ParticleBodyGroup>([&vertices] (ex::Entity entity, Particle &particle, ParticleBodyGroup &bodyGroup)
        {
            const float r = particle.radius;
            for (auto& body : bodyGroup.m_kBodys) {
...
public:
    ...
    void emit_particles(ex::EntityManager &es, ex::Entity entity)
    {
        ...
        sf::Color colour = renderable->shapePtr->getFillColor();
        colour.a = 200;

        ex::Entity particle = es.create();
        float area = (M_PI * collideable->radius * collideable->radius) / 3.0;
        particle.assign<ParticleBodyGroup>((size_t)area,
            body->position,
            body->direction,
            0.0);
        float radius = r(3, 1);
        particle.assign<Particle>(colour, radius, radius / 2);
        ex::ComponentHandle <ParticleBodyGroup> kGroup = particle.component<ParticleBodyGroup>();
        for (auto& kSubBody : kGroup->m_kBodys) {
            float rotationd = r(720, 180, 10);
            if (std::rand() % 2 == 0) rotationd = -rotationd;

            float offset = r(collideable->radius, 1, 1);
            float angle = r(1000, 0, 1) * M_PI / 180.0;

            kSubBody.position = kGroup->position + sf::Vector2f(offset * cos(angle), offset * sin(angle));
            kSubBody.direction = kGroup->direction + sf::Vector2f(offset * 2 * cos(angle), offset * 2 * sin(angle));
            kSubBody.rotationd = rotationd;
        }
    }
...
public:
    ...
    void update(ex::EntityManager &es, ex::EventManager &events, ex::entityx::TimeDelta dt) override
    {
        ...
        {
            renderable.shapePtr->setPosition(body.position);
            renderable.shapePtr->setRotation(body.rotation);
            target.draw(*renderable.shapePtr);
        });
        size_t nParticleCount = 0;
        es.each<ParticleBodyGroup>([&nParticleCount] (ex::Entity entity, ParticleBodyGroup &body)
        {
            nParticleCount += body.m_kBodys.size();
        });
        last_update += dt;
        frame_count++;
        if (last_update >= 0.5) {
            ...
            out << es.size() << " entities, " << nParticleCount << " particles " << "(" << static_cast<int>(fps) << " fps)";
...
public:
    ...
    {
        systems.add<SpawnSystem>(target, 300);
    ...
    void update(ex::entityx::TimeDelta dt)
    {
...
int main()
{
    ...
    sf::RenderWindow window(sf::VideoMode(1366, 768), "EntityX Example", sf::Style::Default);
