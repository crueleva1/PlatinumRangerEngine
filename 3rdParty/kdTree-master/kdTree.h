...
template <typename PointType, typename PointArray=std::vector<PointType> >
 ...
 template <typename F>
 void preoderTraveralTest(F func);
 ...
 void insertPointEmplace(Args&&... _Args);
 ...
 template<int SplitDimension, typename F>
 void preorderTraversalSubtreeTest(F func,
                              std::unique_ptr<KDNode<PointType, SplitDimension> >& node);
...
template<typename PointType, typename PointArray>
  ...
  template<typename PointType, typename PointArray>
  template <typename F>
  void KDTree<PointType, PointArray>::preoderTraveralTest(F func)
  {

      preorderTraversalSubtreeTest<0, F>(func, root);

  }

  template<typename PointType, typename PointArray>
  template<int SplitDimension, typename F>
  void KDTree<PointType, PointArray>::preorderTraversalSubtreeTest(F func,
                                                              std::unique_ptr<KDNode<PointType,
                                                              SplitDimension> >& node)
  {
      auto constexpr nextDimension = (SplitDimension + 1) % PointType::dimension;
      if (node->leftChild) {
          preorderTraversalSubtreeTest<nextDimension, F>(func, node->leftChild);
      }
      if (!func (points[node->treeIndex]))
          return;
      if (node->rightChild) {
          preorderTraversalSubtreeTest<nextDimension, F>(func, node->rightChild);
      }
...
template <typename PointType, typename PointArray>
template <typename ... Args>
  void KDTree<PointType, PointArray>::insertPointEmplace(Args&&... _Args){
  points.emplace_back(_Args...);
  root = insertPointSubtree<0>(root, points.size() - 1);
}
